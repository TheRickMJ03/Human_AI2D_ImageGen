    ...
    if (threeDModel && show3DViewer && !isLoadingLibrariesRef.current) {
      const loadAndRender = async () => {
        ...
        try {
          ...
          let THREE, SplatMesh, SparkRenderer, PointerControls;
          try {
            const threeModule = await import("three");
            THREE = threeModule;
            const sparkModule = await import("@sparkjsdev/spark");
            ({ SplatMesh, SparkRenderer, PointerControls } = sparkModule);
          } catch (err) {
          ...
          // camera stays fixed, slightly back
          ...
          // Position the segmented object correctly
          ...
          // PointerControls only for the object
          ...
          resetViewRef.current = () => {
            if (!splatMesh || !initialState) return;
            splatMesh.position.copy(initialState.position);
            splatMesh.scale.copy(initialState.scale);
            splatMesh.rotation.copy(initialState.rotation);
          };
          ...
          const animate = () => {
            ...
            try {
              if (controlsRef.current && splatMeshRef.current) {
                const deltaTime = 0.016;
                controlsRef.current.update(deltaTime, splatMeshRef.current);
              }
